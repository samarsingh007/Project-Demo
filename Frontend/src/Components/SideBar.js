  ...
  return (
    <div className={`sidebar-container ${isCollapsed ? 'collapsed' : ''}`}>
      <div className="sidebar-header">
        {/* The brand: always show the logo, only show text if not collapsed */}
        <div className="brand">
          <span className="brand-logo">
            <img src={MainLogo} alt="Logo" className="logo-img" />
          </span>
          {!isCollapsed && (
            <span className="brand-text">ParentCoach.AI</span>
          )}
        </div>

        {/* Collapse/expand button */}
        <button className="collapse-btn" onClick={toggleSidebar}>
          {isCollapsed ? <img src={Show} alt="Show" /> : <img src={Hide} alt="Hide" />}
        </button>
      </div>
